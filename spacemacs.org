...
- Kill buffers in Helm Buffer - ~C-x b~ shows helm buffer, select buffers to kill with ~Ctrl-SPC~, kill them with ~M-D~.
** Magit
- ~C-x g~ or ~SPC g s~ - magit status
- ~N~ and ~P~ move around next and previous, ~TAB~ expands/collapses code folds.
- ~s~ stage
- ~u~ unstage
